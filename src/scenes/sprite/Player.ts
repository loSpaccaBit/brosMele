  ...
  implements IPlayer
{
  ...
  private _velocityMin: number = 60;
  private _velocityMax: number = 100;
  ...
  update(time: number, delta: number) {
    ...
    if (this._cursors.right.isDown) {
      ...
      this.anims.play("move", true);
      //setta la velocità x in modo da far muovere il player
      this._body.setVelocityX(this._velocityMin);
      this._body.setMaxVelocity(this._velocityMax, this._velocityMax);
    }
    /*   if (this._cursors.up.isDown) {
      this._body.setAllowGravity(true);
      //this._body.setVelocityY(100);
      // this._body.setGravityY(-700);
      this._body.setGravity(100, -200);
    }
    ...
    if (this._cursors.right.isUp && this._cursors.left.isUp) {
